...
class DDPM(BaseModel):
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if 'res' in ret.keys():
                ...
                self.results['preds']['pred_jts'] = self.results['preds']['pred_jts'] + ret['res']
        a = self.results['preds']['raw_pred_jts']
        b = self.results['preds']['pred_jts']
        c = self.results['res']
        self.netG.train()
    ...
    def validate_gt(self, json_path, heatmap_to_coord, val_data, opt):
        ...
        res = evaluate_mAP(os.path.join(json_path, 'result.json'), ann_type='keypoints', img_ids=img_ids)
