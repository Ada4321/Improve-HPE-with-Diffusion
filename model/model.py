...
class DDPM(BaseModel):
        ...
        if self.opt['phase'] == 'train':
            ...
            if opt['train']['optimizer'] == 'adam':
                self.optG = torch.optim.Adam(
                    optim_params, lr=opt['train']["lr"])
            elif opt['train']['optimizer'] == 'sgd':
                self.optG = torch.optim.SGD(
                    optim_params, lr=opt['train']["lr"], momentum=0.9, weight_decay=0.0001)
    ...
    def validate(self, json_path, heatmap_to_coord, val_loader):
        kpt_json = []
    ...
    def validate_gt(self, json_path, heatmap_to_coord, val_loader):
        kpt_json = []
