...
def mkdirs(paths):
    if isinstance(paths, str): # if it is a string
        os.makedirs(paths, exist_ok=True)
    else:
        for path in paths:  # if it is a list
...
def parse(args):
    ...
    with open(opt_path, 'r') as f:
        for line in f:
            line = line.split('//')[0] + '\n'  # remove annotations
            json_str += line
    opt = json.loads(json_str, object_pairs_hook=OrderedDict)  # convert str to dict

    # set log directory
    # if args.debug:
    #     opt['name'] = 'debug_{}'.format(opt['name'])
    experiments_root = os.path.join(
        '/root/experiments', '{}_{}'.format(opt['name'], get_timestamp()))
